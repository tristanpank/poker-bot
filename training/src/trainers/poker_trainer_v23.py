...
from poker_model_v23 import AdvantageBuffer, PokerDeepCFRNet, StrategyBuffer, load_compatible_state_dict
from poker_state_v23 import (
    ACTION_COUNT_V21,
    ACTION_NAMES_V21,
    OPPONENT_PROFILE_DEFAULT_V23,
    OPPONENT_PROFILE_DEFAULT_SLOT_V23,
    OPPONENT_PROFILE_FEATURE_NAMES_V23,
    OPPONENT_PROFILE_PER_SLOT_DIM_V23,
    POSITION_NAMES_V21,
    STATE_DIM_V21,
)
from poker_worker_v23 import SYNTHETIC_OPPONENT_STYLES, run_policy_hand, run_traversal, run_traversal_batch_mp
from gpu_rollout_inference_v23 import GPURolloutInferenceService
...
class DeepCFRConfig:
    ...
    opponent_profile: tuple = OPPONENT_PROFILE_DEFAULT_V23
...
class DeepCFRTrainerV23:
    def __init__(self, config: Optional[DeepCFRConfig] = None):
        ...
        self._tag_snapshot(self.actor_snapshot, "actor_0")
        ...
        self._held_opponent_snapshot: Optional[PokerDeepCFRNet] = None
    ...
    def _load_v21_seed_template(self) -> Optional[PokerDeepCFRNet]:
        ...
        if not os.path.isfile(path):
            print(f"[v23] Seed checkpoint not found: {path}")
            return None
        ...
        try:
            ...
            print(f"[v23] Failed to load v21 seed checkpoint from '{path}': {exc}")
            return None
    ...
    def _reset_checkpoint_pool(self) -> None:
        ...
        if self._v21_seed_template is not None and seed_count > 0:
            for seed_idx in range(seed_count):
                snapshot = self._v21_seed_template.clone_cpu()
                self._tag_snapshot(snapshot, f"seed_v21_{seed_idx}")
                self.checkpoint_pool.append(snapshot)
        self._tag_snapshot(self.actor_snapshot, f"actor_{int(getattr(self, '_snapshot_version', 0))}")
    ...
    def _publish_actor_snapshot(self) -> None:
        ...
        self._tag_snapshot(self.actor_snapshot, f"actor_{self._snapshot_version}")
    ...
    ) -> tuple[Optional[PokerDeepCFRNet], str, Dict[int, tuple], int, int, str, str]:
    ...
    def _opponent_profile_key_for_snapshot(self, snapshot: PokerDeepCFRNet) -> str:
    ...
    def _opponent_profile_tuple_for_record(self, record: Dict[str, float]) -> tuple:
        ...
        if len(values) != OPPONENT_PROFILE_PER_SLOT_DIM_V23:
            return tuple(OPPONENT_PROFILE_DEFAULT_SLOT_V23)
        return tuple(values)
    ...
    @staticmethod
    def _gpu_model_key_for_snapshot(snapshot: Optional[PokerDeepCFRNet], actor_snapshot: Optional[PokerDeepCFRNet]) -> str:
        if snapshot is None or snapshot is actor_snapshot:
            return "actor_snapshot"
        tagged_key = str(getattr(snapshot, "_gpu_snapshot_key", "") or "")
        if tagged_key:
            return tagged_key
        return (
            f"checkpoint_{id(snapshot)}_"
            f"{int(getattr(snapshot, 'state_dim', 0) or 0)}_"
            f"{int(getattr(snapshot, 'hidden_dim', 0) or 0)}_"
            f"{int(getattr(snapshot, 'action_dim', 0) or 0)}"
        )

    @staticmethod
    def _tag_snapshot(snapshot: Optional[PokerDeepCFRNet], snapshot_key: str) -> None:
        if snapshot is None:
            return
        setattr(snapshot, "_gpu_snapshot_key", str(snapshot_key))

    def _register_gpu_snapshot(self, snapshot: Optional[PokerDeepCFRNet]) -> str:
    ...
    def _gpu_runtime_config(self, actor_key: str, opponent_key: str) -> Dict[str, object]:
        ...
        return {
            ...
            "gpu_actor_state_dim": int(getattr(self.actor_snapshot, "state_dim", self.config.state_dim) or self.config.state_dim),
            "gpu_actor_hidden_dim": int(getattr(self.actor_snapshot, "hidden_dim", self.config.hidden_dim) or self.config.hidden_dim),
            "gpu_actor_action_dim": int(getattr(self.actor_snapshot, "action_dim", self.config.action_count) or self.config.action_count),
    ...
    def _run_parallel_traversals(self, num_traversals: int) -> None:
        ...
        while chunk_start < count:
            ...
            config_dict["opponent_profile"] = tuple(OPPONENT_PROFILE_DEFAULT_V23)
            ...
            if use_gpu_rollout:
                ...
                config_dict["gpu_actor_state_dim"] = int(getattr(self.actor_snapshot, "state_dim", self.config.state_dim) or self.config.state_dim)
                config_dict["gpu_actor_hidden_dim"] = int(getattr(self.actor_snapshot, "hidden_dim", self.config.hidden_dim) or self.config.hidden_dim)
                config_dict["gpu_actor_action_dim"] = int(getattr(self.actor_snapshot, "action_dim", self.config.action_count) or self.config.action_count)
            remote_opponents = bool(use_gpu_rollout and getattr(self.config, "gpu_rollout_remote_opponents", False))
            if opponent_snapshot is None or opponent_snapshot is self.actor_snapshot:
                ...
                if use_gpu_rollout:
                    config_dict["gpu_opponent_state_dim"] = int(getattr(self.actor_snapshot, "state_dim", self.config.state_dim) or self.config.state_dim)
                    config_dict["gpu_opponent_hidden_dim"] = int(getattr(self.actor_snapshot, "hidden_dim", self.config.hidden_dim) or self.config.hidden_dim)
                    config_dict["gpu_opponent_action_dim"] = int(getattr(self.actor_snapshot, "action_dim", self.config.action_count) or self.config.action_count)
            else:
                opponent_model_key = (
                ...
                if use_gpu_rollout:
                    config_dict["gpu_opponent_state_dim"] = int(getattr(opponent_snapshot, "state_dim", self.config.state_dim) or self.config.state_dim)
                    config_dict["gpu_opponent_hidden_dim"] = int(getattr(opponent_snapshot, "hidden_dim", self.config.hidden_dim) or self.config.hidden_dim)
                    config_dict["gpu_opponent_action_dim"] = int(getattr(opponent_snapshot, "action_dim", self.config.action_count) or self.config.action_count)
                opponent_state = (
    ...
    def _evaluate(self, mode: str, num_hands: int) -> EvaluationReport:
        ...
        eval_config.opponent_profile = OPPONENT_PROFILE_DEFAULT_V23
...
DeepCFRTrainerV21 = DeepCFRTrainerV23
