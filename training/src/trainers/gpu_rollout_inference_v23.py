...
def _infer_model_spec_from_state_dict(
...
def _model_input_dim(model: PokerDeepCFRNet) -> int:
    input_layer = getattr(model, "input_layer", None)
    in_features = int(getattr(input_layer, "in_features", 0) or 0)
    if in_features > 0:
        return in_features
    return int(getattr(model, "state_dim", 0) or 0)
...
class GPURolloutInferenceService:
    ...
    def start(self) -> None:
        ...
        self._accept_thread = threading.Thread(target=self._accept_loop, name="v23-gpu-infer-accept", daemon=True)
        self._serve_thread = threading.Thread(target=self._serve_loop, name="v23-gpu-infer-serve", daemon=True)
    ...
    def register_model(
        ...
        inferred_state_dim, inferred_hidden_dim, inferred_action_dim = _infer_model_spec_from_state_dict(
            cpu_state,
            fallback_state_dim=state_dim,
            fallback_hidden_dim=hidden_dim,
            fallback_action_dim=action_dim,
        )
        with self._registry_lock:
            self._cpu_registry[key] = cpu_state
            self._model_specs[key] = (int(inferred_state_dim), int(inferred_hidden_dim), int(inferred_action_dim))
    ...
    def _load_model_to_device(self, model_key: str) -> PokerDeepCFRNet:
        ...
        model.state_dim = _model_input_dim(model)
        model.hidden_dim = int(hidden_dim)
    ...
    def _process_batch(self, batch: List[Tuple[object, dict]]) -> None:
        ...
        for (model_key, heads), items in grouped.items():
            try:
                ...
                expected_dim = _model_input_dim(model)
