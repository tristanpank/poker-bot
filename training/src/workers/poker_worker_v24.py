...
from poker_model_v23 import masked_policy as masked_policy_v23
from poker_model_v24 import PokerDeepCFRNet, load_compatible_state_dict, masked_policy, regret_matching
from poker_state_v23 import (
    ACTION_CALL as LEGACY_ACTION_CALL_V23,
    ACTION_CHECK as LEGACY_ACTION_CHECK_V23,
    ACTION_COUNT_V21 as LEGACY_ACTION_COUNT_V23,
    ACTION_FOLD as LEGACY_ACTION_FOLD_V23,
    ACTION_RAISE_HALF_POT as LEGACY_ACTION_RAISE_HALF_POT_V23,
    ACTION_RAISE_POT_OR_ALL_IN as LEGACY_ACTION_RAISE_POT_OR_ALL_IN_V23,
    OPPONENT_PROFILE_DEFAULT_V23,
    build_legal_action_mask as build_legal_action_mask_v23,
    encode_info_state as encode_info_state_v23,
)
...
def _record_postflop_action_stats(
    ...
    if seat < len(aggression_opportunities):
        ...
        if _is_raise_action(action_id) and seat < len(aggression_counts):
    ...
    if int(getattr(hand_ctx, "current_street", 0)) == 1:
        ...
        if (
            ...
            and _is_raise_action(action_id)
...
def _is_legacy_v23_model(model: Optional[object]) -> bool:
    if model is None:
        return False
    if str(getattr(model, "_policy_family", "") or "") == "legacy_v23":
        return True
    return int(getattr(model, "action_dim", 0) or 0) == int(LEGACY_ACTION_COUNT_V23)


def _map_legacy_v23_action_to_v24_action(state, actor: int, legacy_action_id: int) -> int:
    action_id = int(legacy_action_id)
    if action_id == LEGACY_ACTION_FOLD_V23:
        return ACTION_FOLD
    if action_id == LEGACY_ACTION_CHECK_V23:
        return ACTION_CHECK
    if action_id == LEGACY_ACTION_CALL_V23:
        return ACTION_CALL
    if action_id == LEGACY_ACTION_RAISE_HALF_POT_V23:
        return ACTION_RAISE_HALF_POT
    if action_id == LEGACY_ACTION_RAISE_POT_OR_ALL_IN_V23:
        target = getattr(state, "pot_completion_betting_or_raising_to_amount", None)
        max_raise = getattr(state, "max_completion_betting_or_raising_to_amount", None)
        if max_raise is None:
            max_raise = getattr(state, "max_completion_betting_or_raising_to", None)
        current_target = int(round(float(target))) if target is not None else None
        max_target = int(round(float(max_raise))) if max_raise is not None else None
        if current_target is not None and max_target is not None and current_target >= max_target:
            return ACTION_ALL_IN
        return ACTION_RAISE_POT
    return ACTION_CHECK


def _apply_legacy_v23_action(state, actor: int, action_id: int, hand_ctx: "HandContext") -> bool:
    actor = int(actor)
    requested = int(action_id)
    valid = True
    legal_mask = build_legal_action_mask_v23(state, actor, hand_ctx)
    if requested < 0 or requested >= LEGACY_ACTION_COUNT_V23 or legal_mask[requested] <= 0.5:
        valid = False
    ...
    try:
        if requested == LEGACY_ACTION_FOLD_V23 and state.can_fold():
            state.fold()
            hand_ctx.in_hand[actor] = False
        elif requested in (LEGACY_ACTION_CHECK_V23, LEGACY_ACTION_CALL_V23) and state.can_check_or_call():
            state.check_or_call()
            if hand_ctx.current_street == 0 and float(to_call) > 1e-6 and requested == LEGACY_ACTION_CALL_V23:
                hand_ctx.preflop_call_count += 1
        elif requested in (LEGACY_ACTION_RAISE_HALF_POT_V23, LEGACY_ACTION_RAISE_POT_OR_ALL_IN_V23) and state.can_complete_bet_or_raise_to():
            min_raise, max_raise = _raise_target_bounds(state)
            if requested == LEGACY_ACTION_RAISE_HALF_POT_V23:
...
def _legacy_v23_policy_action(
    ...
    legacy_state_vec, legacy_legal_mask = encode_info_state_v23(
        state,
        actor,
        hand_ctx,
        return_legal_mask=True,
        opponent_profile=opponent_profile if opponent_profile is not None else OPPONENT_PROFILE_DEFAULT_V23,
        opponent_profiles_by_seat=opponent_profiles_by_seat,
    )
    outputs = _forward_outputs(snapshot, legacy_state_vec, heads=("strategy",))
    logits = np.asarray(outputs.get("strategy", np.zeros(LEGACY_ACTION_COUNT_V23, dtype=np.float32)), dtype=np.float32)
    probs = masked_policy_v23(logits, legacy_legal_mask)
    legacy_action = _sample_action(probs, rng)
    return int(legacy_action), _map_legacy_v23_action_to_v24_action(state, actor, legacy_action)
...
def _legal_raise_actions_from_mask(legal_mask: np.ndarray, actions=ALL_RAISE_ACTIONS) -> List[int]:
    return [int(action_id) for action_id in actions if 0 <= int(action_id) < len(legal_mask) and float(legal_mask[int(action_id)]) > 0.5]


def _small_raise_action(legal_mask: np.ndarray) -> Optional[int]:
    legal = _legal_raise_actions_from_mask(legal_mask, SMALL_RAISE_ACTIONS)
    if legal:
        return int(legal[0])
    legal = _legal_raise_actions_from_mask(legal_mask, NON_ALL_IN_RAISE_ACTIONS)
    if legal:
        return int(legal[0])
    legal = _legal_raise_actions_from_mask(legal_mask, (ACTION_ALL_IN,))
    if legal:
        return int(legal[0])
    return None


def _medium_raise_action(legal_mask: np.ndarray) -> Optional[int]:
    legal = _legal_raise_actions_from_mask(legal_mask, MEDIUM_RAISE_ACTIONS)
    if legal:
        return int(legal[0])
    return _small_raise_action(legal_mask)


def _large_raise_action(legal_mask: np.ndarray, include_all_in: bool = True) -> Optional[int]:
    actions = LARGE_RAISE_ACTIONS + ((ACTION_ALL_IN,) if include_all_in else ())
    legal = _legal_raise_actions_from_mask(legal_mask, actions)
    if legal:
        return int(legal[-1])
    legal = _legal_raise_actions_from_mask(legal_mask, NON_ALL_IN_RAISE_ACTIONS)
    if legal:
        return int(legal[-1])
    if include_all_in and float(legal_mask[ACTION_ALL_IN]) > 0.5:
        return ACTION_ALL_IN
    return None


def _boost_action(scores: np.ndarray, legal_mask: np.ndarray, action_id: Optional[int], amount: float) -> bool:
    if action_id is None:
        return False
    idx = int(action_id)
    if idx < 0 or idx >= len(scores) or float(legal_mask[idx]) <= 0.5:
        return False
    scores[idx] += float(amount)
    return True


def _aggressive_action(legal_mask: np.ndarray, prefer_large: bool = False) -> int:
    preferred_raise = _large_raise_action(legal_mask) if prefer_large else _small_raise_action(legal_mask)
    if preferred_raise is not None:
        return int(preferred_raise)
    fallback_raise = _small_raise_action(legal_mask)
    if fallback_raise is not None:
        return int(fallback_raise)
    return _first_legal_action(legal_mask, ACTION_CALL, ACTION_CHECK, ACTION_FOLD)
...
def _passive_action(legal_mask: np.ndarray, facing_wager: bool) -> int:
    if facing_wager:
        fallback_raise = _small_raise_action(legal_mask)
        if fallback_raise is not None:
            return _first_legal_action(legal_mask, ACTION_CALL, ACTION_CHECK, ACTION_FOLD, fallback_raise)
        return _first_legal_action(legal_mask, ACTION_CALL, ACTION_CHECK, ACTION_FOLD)
    fallback_raise = _small_raise_action(legal_mask)
    if fallback_raise is not None:
        return _first_legal_action(legal_mask, ACTION_CHECK, ACTION_CALL, ACTION_FOLD, fallback_raise)
    return _first_legal_action(legal_mask, ACTION_CHECK, ACTION_CALL, ACTION_FOLD)


def _fold_action(legal_mask: np.ndarray) -> int:
    fallback_raise = _small_raise_action(legal_mask)
    if fallback_raise is not None:
        return _first_legal_action(legal_mask, ACTION_FOLD, ACTION_CHECK, ACTION_CALL, fallback_raise)
    return _first_legal_action(legal_mask, ACTION_FOLD, ACTION_CHECK, ACTION_CALL)
...
def _new_exploit_policy_scores(legal_mask: np.ndarray, facing_wager: bool) -> np.ndarray:
    ...
    _boost_action(scores, legal_mask, _small_raise_action(legal_mask), 0.24)
    _boost_action(scores, legal_mask, _medium_raise_action(legal_mask), 0.10)
    _boost_action(scores, legal_mask, _large_raise_action(legal_mask, include_all_in=False), 0.04)
    _boost_action(scores, legal_mask, ACTION_ALL_IN, 0.02)
    return scores
...
def compute_exploit_guidance(
    ...
    can_raise = any(float(legal_mask[action_id]) > 0.5 for action_id in ALL_RAISE_ACTIONS)
    ...
    if street == 0:
        ...
        if preflop_raise_count == 0 and can_raise:
            ...
            if steal_signal > 1e-3:
                ...
                if preflop_strength >= 0.42:
                    _boost_action(scores, legal_mask, _small_raise_action(legal_mask), open_boost)
                    if preflop_strength >= 0.72:
                        _boost_action(scores, legal_mask, _medium_raise_action(legal_mask), 0.18 * steal_signal)
                else:
            ...
            if can_raise:
                ...
                if punish_three_bet_signal > 1e-3 and preflop_strength >= 0.34:
                    _boost_action(
                        scores,
                        legal_mask,
                        _medium_raise_action(legal_mask),
                        float(0.80 + 0.60 * punish_three_bet_signal),
                    )
                    if preflop_strength >= 0.60:
                        _boost_action(scores, legal_mask, _large_raise_action(legal_mask), 0.20 * punish_three_bet_signal)
        ...
        if hero_is_last_aggressor and can_raise and not facing_wager and street in (1, 2):
            ...
            if cbet_signal > 1e-3 and hand_strength < 0.78:
                _boost_action(scores, legal_mask, _small_raise_action(legal_mask), float(0.80 + 0.55 * cbet_signal))
        ...
        if station_signal > 1e-3:
            if hand_strength < 0.50 and not facing_wager:
                ...
                for action_id in ALL_RAISE_ACTIONS:
                    if float(legal_mask[action_id]) > 0.5:
                        scores[action_id] *= 0.55
                ...
                _boost_action(scores, legal_mask, _medium_raise_action(legal_mask), float(0.45 + 0.35 * station_signal))
                if hand_strength >= 0.82:
                    _boost_action(scores, legal_mask, _large_raise_action(legal_mask), 0.22 * station_signal)
...
def _heuristic_action(state, actor: int, hand_ctx: HandContext, rng: random.Random) -> int:
    ...
    if strength >= 0.85:
        strong_raise = _large_raise_action(legal_mask)
        if strong_raise is not None:
            return int(strong_raise)
        if legal_mask[ACTION_CALL] > 0.5:
            return ACTION_CALL
        return ACTION_CHECK

    if strength >= max(0.45, pot_odds + 0.08):
        if to_call <= 0.0 and strength >= 0.62 and rng.random() < 0.55:
            medium_raise = _medium_raise_action(legal_mask)
            if medium_raise is not None:
                return int(medium_raise)
        if legal_mask[ACTION_CALL] > 0.5:
            return ACTION_CALL
...
def apply_abstract_action(state, actor: int, action_id: int, hand_ctx: HandContext) -> bool:
    ...
    try:
        if requested == ACTION_FOLD and state.can_fold():
            ...
            target = abstract_raise_target(state, requested)
            if target is None:
                target = max_raise if requested == ACTION_ALL_IN else None
            if target is None:
                raise ValueError(f"Unable to map abstract raise action {requested}")
...
def _simulate_from_state(
    ...
    traverser_opponent_profile = getattr(config, "opponent_profile", OPPONENT_PROFILE_DEFAULT_V24)
    ...
    while state.status:
        ...
        if actor == traverser:
            ...
            if record and result is not None:
                ...
                if hand_ctx.current_street == 0:
                    ...
                    if chosen_action == ACTION_CALL or _is_raise_action(chosen_action):
                        result.vpip = True
                    if _is_raise_action(chosen_action):
                        result.pfr = True
        ...
        if opponent_policy_mode == "synthetic":
            ...
            action_for_stats = int(chosen_action)
            apply_with_legacy = False
        else:
            policy_model = opponent_snapshot if opponent_snapshot is not None else actor_snapshot
            infer_start = time.perf_counter()
            if _is_legacy_v23_model(policy_model):
                chosen_action, action_for_stats = _legacy_v23_policy_action(
                    policy_model,
                    state,
                    actor,
                    hand_ctx,
                    rng,
                    opponent_profile=getattr(config, "opponent_profile", OPPONENT_PROFILE_DEFAULT_V24),
                    opponent_profiles_by_seat=getattr(config, "opponent_profiles_by_seat", None),
                )
                apply_with_legacy = True
            else:
        ...
        if record and result is not None:
            ...
            if hand_ctx.current_street == 0:
                ...
                _record_postflop_action_stats(result.preflop_stats, actor, hand_ctx, to_call, action_for_stats)
        apply_start = time.perf_counter()
        is_valid = (
            _apply_legacy_v23_action(state, actor, chosen_action, hand_ctx)
            if apply_with_legacy
            else apply_abstract_action(state, actor, chosen_action, hand_ctx)
        )
        perf["apply_time"] += time.perf_counter() - apply_start
        if record and result is not None and not is_valid:
            result.invalid_action_count += 1
    ...
    while state.status:
        ...
        if actor == hero_seat:
            ...
            if opponent_mode == "checkpoints" and seat_models:
                seat_model = seat_models.get(actor)
                if _is_legacy_v23_model(seat_model):
                    chosen_action, action_for_stats = _legacy_v23_policy_action(
                        seat_model,
                        state,
                        actor,
            ...
            if hand_ctx.current_street == 0:
                ...
                _record_postflop_action_stats(preflop_stats, actor, hand_ctx, to_call, action_for_stats)

        is_valid = (
            _apply_legacy_v23_action(state, actor, chosen_action, hand_ctx)
            if actor != hero_seat and use_legacy_apply
            else apply_abstract_action(state, actor, chosen_action, hand_ctx)
        )
        if not is_valid:
...
def _load_model_from_state_dict(
    ...
    target_state_dim = int(state_dim)
    inferred_hidden_dim = int(hidden_dim)
    target_action_dim = int(action_dim)

    input_weight = None
    strategy_out_weight = None
    for key, tensor in state_dict.items():
        name = str(key)
        if input_weight is None and name.endswith("input_layer.weight"):
            input_weight = tensor
        if strategy_out_weight is None and name.endswith("strategy_head.2.weight"):
            strategy_out_weight = tensor
    if input_weight is not None and getattr(input_weight, "ndim", 0) == 2:
        target_state_dim = int(input_weight.shape[1])
        inferred_hidden_dim = int(input_weight.shape[0])
    if strategy_out_weight is not None and getattr(strategy_out_weight, "ndim", 0) == 2:
        target_action_dim = int(strategy_out_weight.shape[0])

    is_legacy_v23 = int(target_action_dim) == int(LEGACY_ACTION_COUNT_V23)

    if is_legacy_v23:
        model = PokerDeepCFRNetV23(
            state_dim=target_state_dim,
            hidden_dim=inferred_hidden_dim,
            action_dim=target_action_dim,
            init_weights=False,
        )
        load_compatible_state_dict_v23(model, state_dict)
        setattr(model, "_policy_family", "legacy_v23")
    else:
        ...
        setattr(model, "_policy_family", "v24")
    model.eval()
    ...
    if quantize_for_cpu_inference:
        ...
        if quantize_dynamic is not None:
            try:
                ...
                quantized.eval()
                setattr(quantized, "state_dim", target_state_dim)
                setattr(quantized, "hidden_dim", inferred_hidden_dim)
                setattr(quantized, "action_dim", target_action_dim)
                setattr(quantized, "_policy_family", "legacy_v23" if is_legacy_v23 else "v24")
                model = quantized
            except Exception:
                pass
    return model
