"""
Poker Worker V17: Self-Play Training for Balanced Strategy

Key improvements over V16:
1. Self-play: Agent plays against past versions of itself
2. Mixed training: 80% self-play + 20% scripted opponents
3. 30 hands per session (longer sessions)
4. Stronger penalties for all-in abuse
"""
...
from poker_model_v17 import (
    DuelingPokerNet, NUM_ACTIONS_V17, OpponentPool,
    ...
    compute_hand_strength_category, compute_v17_session_reward,
    compute_v17_hand_shaping
)
...
# =============================================================================
# Scripted Opponent Classes (same as V15/V16)
# =============================================================================
...
# =============================================================================
# V17 Session-Based Episode Runner with Self-Play
# =============================================================================

def run_session_v17(args):
    """
    Run a multi-hand poker session with self-play.
    
    Key features:
    - 30 hands per session (longer than V16)
    - Self-play: past model snapshots as opponents
    - Mixed mode: 80% self-play, 20% scripted
    - Stronger all-in and bust penalties
    """
    (session_seed, epsilon, equity_iterations, model_state_dict, 
     hands_per_session, opponent_pool_state_dicts, self_play_ratio) = args
    ...
    session_transitions = []
    session_action_counts = {i: 0 for i in range(NUM_ACTIONS_V17)}
    allin_outcomes = []
    hands_completed = 0
    position_history = []
    hand_profits = []
    
    # Decide session type: self-play or scripted
    use_self_play = (len(opponent_pool_state_dicts) > 0 and 
                     random.random() < self_play_ratio)
    
    # Create opponents
    if use_self_play:
        # Create self-play opponents from pool snapshots
        self_play_models = []
        for _ in range(5):  # 5 opponents
    ...
    for hand_num in range(hands_per_session):
        ...
        def scripted_opponent_action(opp_idx: int, player_idx: int, legal: List[int]) -> int:
        ...
        def self_play_opponent_action(opp_idx: int, player_idx: int, legal: List[int]) -> int:
        ...
        while state.status is not False:
            ...
            if current_actor == agent_position:
                ...
                if use_self_play:
                    action = self_play_opponent_action(opp_idx, current_actor, legal)
                else:
                    action = scripted_opponent_action(opp_idx, current_actor, legal)
        ...
        if pending_obs is not None:
            ...
            shaped = compute_v17_hand_shaping(
    ...
    # Compute session-level reward (V17: stronger penalties)
    session_reward = compute_v17_session_reward(
