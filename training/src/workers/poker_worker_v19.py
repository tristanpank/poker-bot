"""
Poker Worker V19: Range-Weighted Monte Carlo Equity

Key improvement over V18:
1. Weighted hand sampling in Monte Carlo equity calculation
   - Opponent hands weighted by starting hand rank (0-100 scale)
...
- Hybrid training (SCRIPTED, SELF_PLAY, MIXED)
...
from poker_model_v19 import (
    DuelingPokerNet, NUM_ACTIONS_V19, OpponentPool, SessionMode,
    ...
    compute_hand_strength_category, compute_v19_session_reward,
    compute_v19_hand_shaping, get_session_mode
)


# =============================================================================
# Hand Strength Rankings (0=AA best, 100=72o worst)
# =============================================================================
...
# Rank ordering for card comparison (highest first)
RANK_ORDER = "AKQJT98765432"


def get_hand_rank(card1: Card, card2: Card) -> int:
    """
    Look up the starting hand rank for two hole cards.
    
    Returns rank 0-100 (0=AA best, 100=72o worst).
    If the hand is not found, returns 100 (worst).
    """
    rank1 = card1.rank
    rank2 = card2.rank
...
def hand_weight(rank: int, street: int) -> float:
    """
    Convert a hand rank (0-100) and street into a sampling weight [0, 1].
    
    - street 0 (preflop): all hands weight 1.0 (no filtering)
    - street 1 (flop): mild filtering (opponents have called/raised preflop)
    - street 2 (turn): moderate filtering
    - street 3 (river): heavy filtering
    
    Uses a sigmoid curve: strong hands → weight ≈ 1.0, 
    weak hands → weight near min_weight.
    
    The "cutoff" parameter controls where the sigmoid centers.
    Lower cutoff = tighter range = only strong hands survive.
    """
    if street == 0:
        return 1.0  # Preflop: no filtering
    
    # Street-dependent parameters
    # cutoff: hand rank at which weight = 0.5
    # steepness: how sharp the sigmoid transition is
    # min_weight: floor so we never fully eliminate a hand
    if street == 1:  # Flop
        cutoff = 60.0
        steepness = 0.08
        min_weight = 0.10
    ...
    # Sigmoid: 1 / (1 + exp(steepness * (rank - cutoff)))
    exponent = steepness * (rank - cutoff)
...
def monte_carlo_equity_multiway(hole_cards: List[Card], board_cards: List[Card], 
                                 num_opponents: int = 1, iterations: int = 20,
                                 street: int = 0) -> float:
    """
    Monte Carlo equity calculation with range-weighted opponent sampling.
    
    When street > 0 (post-flop), opponent hands are filtered using rejection
    sampling based on their starting hand strength. Weaker hands (that realistic
    opponents would have folded preflop) are less likely to be included.
    
    Args:
        hole_cards: Our hole cards
        board_cards: Community cards currently on the board
    ...
    # Use more raw iterations when filtering, since some will be rejected
    ...
    while valid_iterations < iterations and attempts < max_attempts:
        ...
        for _ in range(num_opponents):
            ...
            # Rejection sampling based on hand strength
...
def get_street_from_board(board_cards: List[Card]) -> int:
...
# =============================================================================
# Scripted Opponent Classes (same as V15/V16/V17/V18)
# =============================================================================
...
def run_session_v19(args):
    """
    Run a multi-hand poker session with hybrid training.
    
    V19 key change: Monte Carlo equity uses range-weighted sampling.
    ...
    - Massive bust penalty (-200 BB)
    - Range-weighted equity for realistic post-flop estimates
    """
    ...
    for hand_num in range(hands_per_session):
        ...
        def get_observation(player_idx: int = None) -> Tuple[np.ndarray, Dict]:
            ...
            # V19: determine street for weighted equity
            current_street = get_street_from_board(board)
            ...
            # V19: Pass street to equity function for weighted sampling
            equity = monte_carlo_equity_multiway(hole, board, max(1, active_opponents), 
                                                  equity_iterations, street=current_street)
            ...
            street_enc = [0.0] * 4
            if len(board) == 0: street_enc[0] = 1.0
            elif len(board) == 3: street_enc[1] = 1.0
            elif len(board) == 4: street_enc[2] = 1.0
            else: street_enc[3] = 1.0
            state_vector.extend(street_enc)
        ...
        def scripted_opponent_action(opp: StochasticOpponent, player_idx: int, legal: List[int]) -> int:
            ...
            # V19: Scripted opponents also use weighted equity for consistency
            current_street = get_street_from_board(board)
        ...
        if pending_obs is not None:
            ...
            shaped = compute_v19_hand_shaping(
    ...
    # Compute session-level reward (V19: same massive bust penalty as V18!)
    session_reward = compute_v19_session_reward(
