"""
Poker Worker V16: Session-Based Training for Long-Term Strategy

Key improvements over V15:
1. Multi-hand sessions (20 hands by default)
2. Persistent stack across hands in session
3. Early termination on bust
4. Session-level reward with all-in tracking
5. Bankroll preservation focus
"""
...
from poker_model_v16 import (
    DuelingPokerNet, NUM_ACTIONS_V16,
    ...
    compute_hand_strength_category, compute_v16_session_reward,
    compute_v16_hand_shaping
)
...
# =============================================================================
# V16 Session-Based Episode Runner
# =============================================================================

def run_session_v16(args):
    """
    Run a multi-hand poker session.
    
    Key features:
    - Persistent stack across hands
    - Up to 20 hands per session (or until bust)
    - Session-level reward calculation
    - Tracks all-in outcomes for penalty calculation
    """
    ...
    num_players = 6
    starting_stack_bb = 100.0  # Starting stack in BB
    big_blind = 10
    small_blind = 5
    bust_threshold_bb = 5.0
    
    # Session state
    agent_stack = starting_stack_bb * big_blind  # Convert to chips
    session_transitions = []
    session_action_counts = {i: 0 for i in range(NUM_ACTIONS_V16)}
    allin_outcomes = []  # Track {'won': bool, 'equity': float}
    hands_completed = 0
    position_history = []
    hand_profits = []
    ...
    for hand_num in range(hands_per_session):
        ...
        # Create new opponents each hand (shuffle seating but keep types)
        opponents = [create_opponent(t) for t in opponent_types]
        random.shuffle(opponents)
        
        # Create game state with current stacks
        # For simplicity, opponents always have full stack
        ...
        try:
            ...
            # Stack too small to post blinds
            break
        ...
        if pending_obs is not None:
            ...
            # Shape reward per hand
            shaped = compute_v16_hand_shaping(
    ...
    session_reward = compute_v16_session_reward(
    ...
    # Update terminal transitions with session reward contribution
    # Distribute session-level bonuses/penalties across terminal states
    terminal_count = sum(1 for t in session_transitions if t[4])  # Count terminal states
    if terminal_count > 0:
        session_bonus_per_hand = (session_reward - session_profit_bb) / terminal_count
        updated_transitions = []
        for t in session_transitions:
            if t[4]:  # Terminal state
