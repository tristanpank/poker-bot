...
from poker_trainer_v21 import DeepCFRTrainerV21
...
DEFAULT_CHECKPOINT_PATH = os.path.join(PROJECT_ROOT, "models", "poker_agent_v21_deepcfr.pt")
...
class TrainingGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Poker Agent V21 - Training Dashboard")
    ...
    def _build_eval_tab(self):
        ...
        self.eval_mode = ttk.Combobox(ctrl, values=["heuristics", "checkpoints"], width=12, state="readonly")
    ...
    def _ingest_snapshot(self, snapshot) -> None:
        ...
        speed = snapshot.hands_per_second
        remaining = (
    ...
    def _show_eval_results(self, results) -> None:
        ...
        lines.append("")
        lines.append(f"{'-' * 40}")
        lines.append("  PREFLOP VPIP BY POSITION")
        lines.append(f"{'-' * 40}")
        for pos_name in POSITION_NAMES:
            lines.append(f"  {pos_name:4s} {results.vpip_by_position.get(pos_name, 0.0):7.1%}")
        ...
        if self.eval_ax is not None:
            ...
            vpip_values = [results.vpip_by_position.get(pos, 0.0) * 100.0 for pos in POSITION_NAMES]
            vpip_bars = self.eval_vpip_ax.bar(
                POSITION_NAMES,
                vpip_values,
                color=ACCENT_COLORS[: len(POSITION_NAMES)],
                edgecolor="none",
            )
            self.eval_vpip_ax.set_ylabel("VPIP %", color=DARK_FG, fontsize=9)
            self.eval_vpip_ax.set_title("Preflop VPIP By Position", color=DARK_FG, fontsize=10)
            self.eval_vpip_ax.tick_params(colors=DARK_FG, labelsize=8)
            for spine in self.eval_vpip_ax.spines.values():
                spine.set_color(DARK_GRID)
            for bar, pct in zip(vpip_bars, vpip_values):
            ...
            grid = np.asarray(results.vpip_hand_grid, dtype=np.float32) * 100.0
            if grid.shape == (13, 13):
                ...
                self.eval_hand_ax.set_title("Overall VPIP by Starting Hand", color=DARK_FG, fontsize=10)
            else:
                self.eval_hand_ax.set_title("Overall VPIP by Starting Hand (No Data)", color=DARK_FG, fontsize=10)
                self.eval_hand_ax.set_xticks([])
                self.eval_hand_ax.set_yticks([])
            self.eval_fig.tight_layout()
            self.eval_canvas.draw()
        else:
            self._draw_eval_fallback(results.action_histogram, results.vpip_by_position, results.vpip_hand_grid)
    ...
    def _draw_heatmap_chart(self, canvas: tk.Canvas, grid_values) -> None:
        frame = self._draw_chart_frame(canvas)
        if frame is None:
            return
        width, height, left, top, right, bottom = frame
        ...
        for idx, rank in enumerate(RANK_LABELS):
            ...
            canvas.create_text(left - 10, top + idx * cell_h + cell_h / 2, text=rank, fill=DARK_FG, font=("Consolas", 7))

    def _draw_eval_fallback(self, counts, vpip_by_position, vpip_hand_grid) -> None:
        ...
        vpip_values = [float(vpip_by_position.get(pos, 0.0)) * 100.0 for pos in POSITION_NAMES]
        self._draw_bar_chart(vpip_canvas, POSITION_NAMES, vpip_values)
        self._draw_heatmap_chart(hand_canvas, vpip_hand_grid)
