...
def _action_palette() -> list[str]:
    return [ACCENT_COLORS[idx % len(ACCENT_COLORS)] for idx in range(len(ACTION_NAMES))]
...
class TrainingGUI(tk.Tk):
    ...
    def _build_eval_tab(self):
        ...
        self.eval_mode = ttk.Combobox(ctrl, values=EVAL_MODE_VALUES, width=14, state="readonly")
    ...
    def _show_eval_results(self, results) -> None:
        ...
        if results is None:
            return
        is_suite = hasattr(results, "leak_reports") and hasattr(results, "robust_reports")
        self._last_eval_report = None if is_suite else results
        ...
        if is_suite:
            ...
            lines = [
                ...
                f"  Avg Leak Profit:  {results.avg_leak_profit_bb:+.3f} BB",
                f"  Avg Robust Profit:{results.avg_robust_profit_bb:+.3f} BB",
                f"  Leak Win Rate:    {results.avg_leak_win_rate:.1%}",
                f"  Robust Win Rate:  {results.avg_robust_win_rate:.1%}",
                "",
                f"{'-' * 40}",
                "  LEAK BOTS",
                f"{'-' * 40}",
            ...
            if self.eval_ax is not None:
                for ax in [self.eval_ax, self.eval_vpip_ax, self.eval_hand_ax]:
                    ...
                    ax.set_xticks([])
                    ax.set_yticks([])
                    ax.text(0.5, 0.5, "Exploit suite summary in text panel", color=DARK_FG, ha="center", va="center")
                    for spine in ax.spines.values():
                        spine.set_color(DARK_GRID)
                self.eval_canvas.draw()
            else:
